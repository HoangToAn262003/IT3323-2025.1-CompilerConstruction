...
Type* compileLValue(void) {
  ...
  switch (var->kind) {
  case OBJ_VARIABLE:
    // NEW: push the variable address onto the stack
...
void compileAssignSt(void) {
  // NEW: Generate code for the assignment
  Type* varType;
  Type* expType;
...
void compileIfSt(void) {
  // NEW: generate code for if-statement
  Instruction *FJ, *J;
...
void compileWhileSt(void) {
  // NEW: generate code for while statement
  Instruction *FJ;
  CodeAddress startAddr = getCurrentCodeAddress();
...
void compileForSt(void) {
  // NEW: generate code for for-statement
  Type* varType;
  Type *type;
  Instruction *FJ;
  CodeAddress L1;
  ...
  eat(KW_DO);
  compileStatement();
  genCV(); genCV(); genLI(); genLC(1); genAD(); genST();
...
void compileCondition(void) {
  // NEW: generate code for condition
  Type* type1;
  Type* type2;
  TokenType op;
...
Type* compileExpression(void) {
  // NEW: generate code for expression
  Type* type;
...
Type* compileExpression3(Type* argType1) {
  // NEW: generate code for expression3
  Type* argType2;
  Type* resultType;
