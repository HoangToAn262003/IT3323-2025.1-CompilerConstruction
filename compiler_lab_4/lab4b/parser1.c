...
Type* compileLValue(void) {
  ...
  switch (var->kind) {
  case OBJ_VARIABLE:
    genVariableAddress(var); // Push variable address onto stack
    ...
    break;
  case OBJ_PARAMETER:
    // Với tham số, nếu là tham trị hay tham biến cần xử lý khác nhau, nhưng cơ bản ta cần địa chỉ
    if (var->paramAttrs->kind == PARAM_VALUE) {
        genVariableAddress(var);
    } else {
        genVariableValue(var); // Tham biến: Giá trị của nó là địa chỉ
    }
    varType = var->paramAttrs->type;
    break;
  case OBJ_FUNCTION:
    // Gán giá trị trả về cho tên hàm (trong thân hàm)
    genVariableAddress(var);
...
void compileAssignSt(void) {
  ...
  genST(); // Store the value from stack to address
}
...
void compileCallSt(void) {
  ...
  if (isPredefinedProcedure(proc)) {
    ...
    compileArguments(proc->procAttrs->paramList);
    // Tính mức (level difference) nếu cần, ở đây giả sử compiler tự tính trong genCALL
    // Hoặc ta cần traverse scopes. 
    // Theo thiết kế IT3323, genCALL(level, address).
    int diff = 0;
    Scope* scope = symtab->currentScope;
    while (scope != proc->procAttrs->scope->outer && scope != NULL) {
        scope = scope->outer;
        diff++;
    }
    genCALL(diff, proc->procAttrs->codeAddress);
  }
}
...
void compileWhileSt(void) {
  ...
  updateFJ(jmpFJ, getCurrentCodeAddress()); // End of loop
}
...
Type* compileIndexes(Type* arrayType) {
  ...
  while (lookAhead->tokenType == SB_LSEL) {
    ...
    // Tính toán offset: index * sizeOfElement
    genLC(sizeOfType(arrayType->elementType));
    genML();
    genAD(); // Cộng vào địa chỉ cơ sở
