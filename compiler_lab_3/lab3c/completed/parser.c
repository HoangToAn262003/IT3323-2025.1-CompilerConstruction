...
void compileBlock(void) {
  ...
  if (lookAhead->tokenType == KW_CONST) {
    ...
    do {
      eat(TK_IDENT);
      // NEW: Check if a constant identifier is fresh in the block
...
void compileBlock2(void) {
  ...
  if (lookAhead->tokenType == KW_TYPE) {
    ...
    do {
      eat(TK_IDENT);
      // NEW: Check if a type identifier is fresh in the block
      checkFreshIdent(currentToken->string);
...
void compileBlock3(void) {
  ...
  if (lookAhead->tokenType == KW_VAR) {
    ...
    do {
      eat(TK_IDENT);
      // NEW: Check if a variable identifier is fresh in the block
      checkFreshIdent(currentToken->string);
...
void compileFuncDecl(void) {
  ...
  eat(KW_FUNCTION);
  eat(TK_IDENT);
  // NEW: Check if a function identifier is fresh in the block
  checkFreshIdent(currentToken->string);
...
void compileProcDecl(void) {
  ...
  eat(KW_PROCEDURE);
  eat(TK_IDENT);
  // NEW: Check if a procedure identifier is fresh in the block
  checkFreshIdent(currentToken->string);
...
ConstantValue* compileUnsignedConstant(void) {
  ...
  case TK_IDENT:
    eat(TK_IDENT);
    // NEW: check if the constant identifier is declared and get its value
...
ConstantValue* compileConstant2(void) {
  ...
  case TK_IDENT:
    eat(TK_IDENT);
    // NEW: check if the integer constant identifier is declared and get its value
    obj = checkDeclaredConstant(currentToken->string);
    constValue = obj->constAttrs->value;
    break;
  default:
...
Type* compileType(void) {
  ...
  case TK_IDENT:
    eat(TK_IDENT);
    // NEW: check if the type idntifier is declared and get its actual type
...
void compileParam(void) {
  ...
  eat(TK_IDENT);
  // NEW: check if the parameter identifier is fresh in the block
  checkFreshIdent(currentToken->string);
...
void compileCallSt(void) {
  eat(KW_CALL);
  eat(TK_IDENT);
  // NEW: check if the identifier is a declared procedure
...
void compileForSt(void) {
  ...
  // NEW: check if the identifier is a variable
