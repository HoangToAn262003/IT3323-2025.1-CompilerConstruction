...
Type* compileLValue(void) {
  ...
  else if (var->kind == OBJ_FUNCTION) varType = var->funcAttrs->returnType;
  else if (var->kind == OBJ_PARAMETER) varType = var->paramAttrs->type;
...
void compileArguments(ObjectNode* paramList) {
  ...
  case SB_LPAR:
    ...
    while (lookAhead->tokenType == SB_COMMA) {
      ...
      if (paramList != NULL) compileArgument(paramList->object);
...
Type* compileFactor(void) {
  ...
  case TK_IDENT:
    ...
    if (obj == NULL) {
      ...
      switch (obj->kind) {
      case OBJ_CONSTANT:
        if (obj->constAttrs->value->type == TP_INT) type = makeIntType();
        else if (obj->constAttrs->value->type == TP_CHAR) type = makeCharType();
        else error(ERR_INVALID_CONSTANT, currentToken->lineNo, currentToken->colNo);
