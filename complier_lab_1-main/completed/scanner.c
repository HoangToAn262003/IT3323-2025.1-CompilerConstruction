...
{
  int tt = 0;
  while ((tt < 2) && (currentChar != EOF))
  {
    int curCharCode = charCodes[currentChar];
    if (curCharCode == CHAR_TIMES)
    {
      tt = 1;
    }
    else if (curCharCode == CHAR_RPAR)
    {
      if (tt == 1)
        tt = 2;
      else
...
Token *readIdentKeyword(void)
{
  ...
  while (((charCodes[currentChar] == CHAR_LETTER || charCodes[currentChar] == CHAR_DIGIT)) && (currentChar != EOF))
  {
    if (cnt <= MAX_IDENT_LEN)
      tk->string[cnt++] = (char)currentChar;
    readChar();
  }

  if (cnt > MAX_IDENT_LEN)
  {
    error(ERR_IDENTTOOLONG, tk->lineNo, tk->colNo);
    return tk;
  }
...
Token *getToken(void)
{
  ...
  case CHAR_LPAR:
    ...
    int curCharCode = charCodes[currentChar];
    if (curCharCode == CHAR_PERIOD)
    {
      readChar();
      return makeToken(SB_LSEL, ln, cn);
    }
    else if (curCharCode == CHAR_TIMES)
    {
      readChar();
      skipComment();
      return getToken();
